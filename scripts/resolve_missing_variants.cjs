...
async function main() {
  console.log("=== RESOLVING ALL MISSING REGOS PRODUCT VARIANTS ===");
  ...
  // Find groups that have missing items
  const pendingGroups = [];
  groups.forEach(g => {
    const missingItems = g.items.filter(it => {
      const hit = cache[it.barcode] || cache[it.sku];
      return !hit || !hit.nameUz || hit.nameUz.includes("Variant") || hit.nameUz.includes("1-turi");
    });
  ...
  console.log(`Found ${pendingGroups.length} groups with ${totalMissingItems} missing barcode variants.`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log(`\n[Chunk ${i + 1}/${chunks.length}] Processing ${batch.length} product groups...`);
...
[
    ...
    while (retries > 0 && !success) {
      try {
        ...
        if (Array.isArray(items)) {
          ...
          success = true;
          console.log(`✓ Resolved and cached ${items.length} variants.`);
        }
      } catch (err) {
        console.warn(`Attempt failed: ${err.message}. Retrying in 2s...`);
        retries--;
        await sleep(2000);
      }
    }
  ...
  console.log("\n🎉 ALL MISSING VARIANTS RESOLVED SUCCESSFULLY!");
  console.log(`Total items in cache: ${Object.keys(cache).length}`);
}
