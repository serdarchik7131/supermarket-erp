  ...
  const handleAdminLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const matchedStaff = staffList.find((st) => {
        const cleanStLogin = (st.login || '').toLowerCase().trim();
        const cleanStPhone = (st.phone || '').replace(/\D/g, '');
        const cleanInput = u.replace(/\D/g, '');
        return (
          cleanStLogin === u ||
          (cleanInput.length >= 7 && cleanStPhone.endsWith(cleanInput.slice(-7)))
        );
      });
      ...
      if (isSuperAdminLogin) {
        isValid = commonAdminPasswords.includes(p) || p === settings.adminPin || p === '123';
