...
export const TelegramApp: React.FC<TelegramAppProps> = ({ initialTab, onOpenAdmin, onOpenAgentPanel }) => {
  ...
  // Debounce search input by 40ms for instant 120 FPS typing
  useEffect(() => {
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : ['Sut', 'Kolbasa', 'Shokolad', 'Pechenye'];
    } catch {
      return ['Sut', 'Kolbasa', 'Shokolad', 'Pechenye'];
    }
  });
  const [sortBy, setSortBy] = useState<'default' | 'price_asc' | 'price_desc' | 'promo'>('default');
  ...
  // Reset display limit when filters change for smooth instant UI
  useEffect(() => {
    setDisplayLimit(36);
  ...
  // Extract unique available brands for quick filtering
  ...
  // Filter & Sort products: Qoldig'i 0 bo'lgan mahsulotlar mijozga ko'rinmaydi (faqat qoldig'i > 0 bo'lganlar chiqadi)
  const filteredProducts = React.useMemo(() => {
    let result = filterProductsSmart(
      products,
      searchQuery,
      selectedCategory,
      categories,
      (p) => {
        ...
        // Qoldiq tekshiruvi: 0 qoldiqli tovarlar yashiriladi
        const currentStock = getTotalStock(p, selectedBranch?.id);
        return currentStock > 0;
      }
    );

    // Apply Sorting only if custom sort requested (otherwise smart relevance sort is preserved)
    if (sortBy === 'price_asc') {
      ...
      result = [...result].sort((a, b) => {
        const aPromo = (a.isPromotional || (a.discountPrice && a.discountPrice > 0)) ? 1 : 0;
        const bPromo = (b.isPromotional || (b.discountPrice && b.discountPrice > 0)) ? 1 : 0;
        return bPromo - aPromo;
      });
    }

    return result;
  }, [products, selectedCategory, searchQuery, selectedBrand, sortBy, categories, selectedBranch]);

  // Chunked visible products slice for ultra-fast rendering (0 lag)
  const displayedProducts = React.useMemo(() => {
    return filteredProducts.slice(0, displayLimit);
  }, [filteredProducts, displayLimit]);

  // Suggested popular products when search returns 0 results
  const suggestedProducts = React.useMemo(() => {
    return products
      .filter((p) => getTotalStock(p, selectedBranch?.id) > 0)
      .slice(0, 4);
  }, [products, selectedBranch]);

  // Helper to determine if a product has fractional quantity (weight/volume like kg, litr, gr) vs discrete pieces (dona, ta, quti, pachka, etc.)
  const isFractionalProduct = (productOrUnit?: Product | string | null): boolean => {
    ...
    // Piece units start from 1: 'dona', 'шт', 'sht', 'pachka', 'quti', 'kor', 'korobka', 'blok', 'ta'
    if (u === 'dona' || u === 'шт' || u === 'sht' || u === 'pachka' || u === 'quti' || u === 'kor' || u === 'korobka' || u === 'blok' || u === 'ta') {
      return false;
    }
    // All other units (kg, кг, l, litr, gr, metr, etc.) are fractional (100gr / 0.1 min)
    return true;
  };
  ...
  // Format display helper for items (supports both piece and weight/volume)
  const formatQuantityDisplay = (qty: number, unit?: string) => {
    if (isFractionalProduct(unit)) {
      const u = (unit || 'kg').toLowerCase().trim();
      const isLitre = u.includes('l') || u.includes('литр');
      const unitLabel = isLitre ? 'L' : (unit || 'kg');
      return `${qty} ${unitLabel}`;
    }
    return `${qty} ${unit || 'dona'}`;
  };

  const openProductModal = (product: Product, initialQty?: number) => {
    setSelectedProductModal(product);
    setModalSize(product.sizes && product.sizes.length > 0 ? product.sizes[0] : '');
    setModalColor(product.colors && product.colors.length > 0 ? product.colors[0] : '');
    const currentInCart = getItemQtyInCart(product.id);
    const minQ = getProductMinQty(product);
    const qty = initialQty !== undefined ? initialQty : (currentInCart > 0 ? currentInCart : minQ);
    setModalQty(qty);
  };
  ...
  const addToCart = (product: Product, size?: string, color?: string, qtyToAdd?: number) => {
    ...
    const defaultQty = qtyToAdd ? Math.max(minQ, qtyToAdd) : minQ;
    ...
    setCart((prev) => {
      ...
      return [...prev, { product, quantity: Math.round(defaultQty * 1000) / 1000, selectedSize: selSize, selectedColor: selColor }];
    });
  };
      ...
      prev
        .map((item) => {
          if (
            ...
            // Only remove from cart if user clicks minus while already at minimal threshold (0.1 for weight or 1 for piece)
      ...
      prev
        .map((item) => {
          if (
            ...
            const minQ = getProductMinQty(item.product);
            const validQty = Math.max(minQ, Math.round(targetQty * 1000) / 1000);
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-100 text-slate-900 justify-between max-w-md mx-auto shadow-2xl relative font-sans overflow-hidden">
      ...
      {/* 1. Compact Emerald Header Bar */}
      <header className="bg-emerald-900 text-white shrink-0 border-b border-emerald-800 px-3 py-2 shadow-sm">
        <div className="flex items-center justify-between gap-2">
          <div
            onClick={handleSecretLogoClick}
            className="flex items-center gap-2 cursor-pointer select-none active:scale-95 transition-transform min-w-0"
            title="Do'kon ma'lumotlari"
          >
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-400 via-emerald-500 to-amber-300 p-0.5 shadow-sm shrink-0">
              <div className="w-full h-full rounded-[10px] bg-emerald-950 flex items-center justify-center font-black text-emerald-400 text-sm">
                🏬
              </div>
            </div>
            <div className="min-w-0">
              <h1 className="font-extrabold text-xs text-white tracking-tight flex items-center gap-1">
                ...
                {storeSettings.storeBadge && (
                  <span className="bg-emerald-500 text-emerald-950 text-[8px] px-1 py-0.2 rounded font-black uppercase shrink-0">
                    {storeSettings.storeBadge}
                  </span>
                )}
              </h1>
              <div className="flex items-center gap-1 text-[10px] text-emerald-200 truncate">
                <MapPin className="w-3 h-3 text-emerald-400 shrink-0" />
                <span className="truncate max-w-[130px] font-medium">{deliveryAddress}</span>
                <span className="text-[9px] text-emerald-300 font-mono font-bold bg-emerald-950/80 px-1 rounded shrink-0">~20m</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1 bg-emerald-800 text-amber-300 border border-emerald-700 px-2 py-0.5 rounded-full text-[11px] font-bold hover:bg-emerald-700 transition-all"
            >
        ...
        {staffMember && (
          <div className="mt-1.5 bg-emerald-950 p-1.5 rounded-lg border border-emerald-700 flex items-center justify-between gap-2 text-xs">
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-ping shrink-0" />
              <div className="truncate">
                <span className="font-black text-emerald-300 text-[10px] block truncate">
                  Xodim: {staffMember.name} ({staffMember.role === 'sales_agent' ? 'Savdo Agenti' : staffMember.role === 'courier' ? 'Kuryer' : staffMember.role === 'accountant' ? 'Buhgalter' : 'Menejer'})
                </span>
              </div>
            </div>
            <button
              onClick={() => setActiveTab(activeTab === 'staff_app' ? 'catalog' : 'staff_app')}
              className="bg-emerald-500 hover:bg-emerald-400 text-emerald-950 px-2 py-0.5 rounded font-black text-[9px] shrink-0 uppercase shadow-xs"
            >
      ...
      <main className="flex-1 overflow-y-auto pb-20">
        ...
        {activeTab === 'catalog' && (
          <div className="p-2 space-y-1.5">
            {/* Phone Check / Quick Phone Identifier Bar (Super Compact) */}
            <div className="bg-white px-2.5 py-1 rounded-xl border border-emerald-100 shadow-2xs flex items-center justify-between text-[11px] gap-2">
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                <input
                  ...
                  className="bg-transparent font-mono font-bold text-emerald-950 focus:outline-none w-full text-[11px]"
                />
              </div>
              <span className="text-[9px] text-emerald-800 bg-emerald-100/90 font-black px-2 py-0.2 rounded-full shrink-0 border border-emerald-200">
                {staffMember ? `Xodim: ${staffMember.role}` : 'Xaridor'}
              </span>
            </div>

            {/* Search Bar with Clear & Voice (Compact) */}
            <div className="relative">
              <div className="relative flex items-center">
                <Search className="w-3.5 h-3.5 absolute left-3 text-emerald-600 pointer-events-none" />
                <input
                  type="text"
                  autoComplete="off"
                  autoCorrect="off"
                  autoCapitalize="off"
                  spellCheck={false}
                  placeholder="Nima qidiryapsiz? (Sut, Kolbasa, Pechenye)..."
                  value={searchInput}
                  ...
                  className="w-full bg-white text-xs font-semibold text-slate-900 pl-8 pr-14 py-1.5 rounded-xl border border-emerald-200 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-500/20 shadow-2xs transition-all placeholder:text-slate-400"
                />
                <div className="absolute right-1.5 flex items-center gap-0.5">
                  ...
                  <button
                    ...
                    className="p-1 text-emerald-600 hover:text-emerald-800 rounded-md hover:bg-emerald-50 transition-colors"
              ...
              {/* Quick Search Dropdown / Recent & Trending searches */}
              {isSearchFocused && !searchInput && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-emerald-200 rounded-xl p-2.5 shadow-xl z-30 space-y-2.5 animate-in fade-in slide-in-from-top-1 duration-150">
                  {/* Recent Searches */}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {recentSearches.map((term) => (
                          <div
                            ...
                            className="group bg-slate-50 hover:bg-emerald-50 text-slate-700 hover:text-emerald-800 text-[10px] font-medium px-2 py-0.5 rounded-md border border-slate-200 hover:border-emerald-300 flex items-center gap-1 cursor-pointer transition-colors"
                          >
                            <span>{term}</span>
                            <button
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {['Kolbasa', 'Sut', 'Pechenye', 'Shokolad', 'Sharbat', 'KDV Bondi'].map((tag) => (
                        <button
                          key={tag}
                          onClick={() => {
                            setSearchQuery(tag);
                          ...
                          className="bg-emerald-50/70 hover:bg-emerald-100 text-emerald-900 text-[10px] font-semibold px-2 py-0.5 rounded-md border border-emerald-200/80 transition-colors"
                        >
                          #{tag}
                        </button>
                      ))}
                  ...
                  <button
                    ...
                    className="w-full text-center text-[10px] font-bold text-emerald-600 pt-1 border-t border-slate-100"
                  >
                    Yopish ✕
                  </button>
                </div>
            ...
            {/* Categories Scroll with Category-Counts & Modal Trigger (Compact No-Scrollbar) */}
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => setIsCategoryModalOpen(true)}
                className="p-1.5 rounded-lg bg-white border border-emerald-200 text-emerald-800 hover:bg-emerald-50 shadow-2xs flex items-center justify-center shrink-0"
                title="Barcha toifalar jadvali"
              >
              ...
              <div className="flex gap-1 overflow-x-auto no-scrollbar py-0.5 flex-1">
                ...
                >
                  ...
                  <span className={`text-[9px] px-1 py-0.2 rounded-full font-bold ${
                    selectedCategory === 'all' ? 'bg-emerald-700 text-emerald-100' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {products.filter(p => getTotalStock(p, selectedBranch?.id) > 0).length}
                  </span>
                </button>
                {categories.map((c) => {
                  const catCount = products.filter(
                    (p) => p.categoryId === c.id && getTotalStock(p, selectedBranch?.id) > 0
                  ).length;
                  return (
                    <button
                      key={c.id}
                    ...
                    >
                      ...
                      {catCount > 0 && (
                        <span className={`text-[9px] px-1 py-0.2 rounded-full font-bold ${
                          selectedCategory === c.id ? 'bg-emerald-700 text-emerald-100' : 'bg-slate-100 text-slate-600'
                        }`}>
                          {catCount}
                        </span>
                      )}
            ...
            {/* Popular Brands Quick Filter Chips (Compact No-Scrollbar) */}
            {availableBrands.length > 1 && (
              <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-0.5">
                <span className="text-[9px] font-extrabold text-slate-400 shrink-0 uppercase tracking-tight">Brend:</span>
                <button
                  onClick={() => setSelectedBrand('all')}
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md whitespace-nowrap transition-all ${
                    selectedBrand === 'all'
                ...
                {availableBrands.slice(0, 8).map((brand) => (
                  <button
                    ...
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md whitespace-nowrap border transition-all ${
                      selectedBrand === brand
            ...
            {/* Filter & View Mode Bar (Sort, Counter, Grid/List toggle - Slim Inline) */}
            <div className="flex items-center justify-between px-1 py-0.5 text-xs">
              <div className="text-[11px] font-bold text-slate-600">
                Topildi: <span className="text-emerald-700 font-extrabold">{filteredProducts.length}</span> ta tovar
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <select
                    ...
                    className="bg-white border border-slate-200 text-slate-800 text-[10px] font-bold rounded-lg px-1.5 py-0.5 focus:outline-none focus:border-emerald-600 shadow-2xs"
                  >
                    ...
                    <option value="promo">Chegirmalar</option>
                  </select>
                </div>
                ...
                <div className="flex items-center bg-white p-0.5 rounded-lg border border-slate-200 shadow-2xs">
                  <button
                    ...
                    className={`p-0.5 rounded ${viewMode === 'grid' ? 'bg-emerald-600 text-white shadow-2xs' : 'text-slate-500'}`}
                    title="2x2 Katak"
                  >
                  ...
                  <button
                    ...
                    className={`p-0.5 rounded ${viewMode === 'list' ? 'bg-emerald-600 text-white shadow-2xs' : 'text-slate-500'}`}
                    title="Ro'yxat"
                  >
            ...
            {isLoadingProducts ? (
              ...
              <div className="space-y-4">
                <div className="bg-white rounded-2xl border border-emerald-200/80 p-6 text-center space-y-2.5 shadow-xs">
                  ...
                  <p className="text-xs text-slate-500 max-w-xs mx-auto">
                    "{searchQuery}" bo'yicha mahsulot topilmadi. Qidiruv so'zini tekshirib ko'ring yoki boshqa toifani tanlang.
                  </p>
                  <div className="flex justify-center gap-2 pt-1">
                    <button
                      onClick={() => { setSearchQuery(''); setSelectedCategory('all'); setSelectedBrand('all'); }}
                      className="bg-emerald-600 text-white hover:bg-emerald-700 font-bold px-4 py-2 rounded-xl text-xs shadow-sm active:scale-95 transition-all"
                    >
                ...
                {/* Smart Suggested Popular Products */}
                {suggestedProducts.length > 0 && (
                  <div className="space-y-2 pt-1">
                    <h4 className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                      Sizga yoqishi mumkin bo'lgan mahsulotlar:
                    </h4>
                    <div className="grid grid-cols-2 gap-2.5">
                      {suggestedProducts.map((p, idx) => {
                        ...
                        return (
                          <div
                            key={`${p.id}_${idx}`}
                            className="bg-white border border-emerald-200/90 rounded-2xl p-2.5 flex flex-col justify-between shadow-2xs hover:shadow-sm"
                          >
                            <div
                              className="cursor-pointer"
                              onClick={() => openProductModal(p)}
                            >
                              <div className="w-full h-24 rounded-xl overflow-hidden mb-1.5 bg-slate-50 relative border border-slate-200 flex items-center justify-center p-1">
                                <ProductThumbnail product={p} iconSize="w-8 h-8" />
                              </div>
                              <h5 className="text-xs font-bold text-slate-900 line-clamp-1">{getProductName(p)}</h5>
                            ...
                            <button
                              onClick={() => {
                                if (isFractionalProduct(p)) {
                                  openProductModal(p);
                                } else {
                                  addToCart(p);
                                }
                              }}
                              className="mt-2 w-full bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold py-1.5 rounded-lg text-[11px] border border-emerald-200 flex items-center justify-center gap-1"
                            >
                              <Plus className="w-3 h-3" />
                              <span>{isFractionalProduct(p) ? 'Gramm / Kg' : 'Savatga'}</span>
                            </button>
                          </div>
                        );
                      })}
              ...
              <div className="space-y-2">
                {displayedProducts.map((p, idx) => {
                  ...
                  const isFract = isFractionalProduct(p);

                  return (
                    <div
                      key={`${p.id}_${idx}`}
                      className="bg-white border border-emerald-200/80 rounded-2xl p-2.5 flex items-center justify-between shadow-2xs hover:border-emerald-400 gap-3 transition-all"
                    >
                      <div
                        className="flex items-center gap-2.5 flex-1 min-w-0 cursor-pointer"
                        onClick={() => openProductModal(p)}
                      >
                        <div className="w-16 h-16 rounded-xl overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center p-1 shrink-0">
                          <ProductThumbnail product={p} iconSize="w-6 h-6" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <span className="text-[9px] text-slate-500 font-mono block">{p.sku}</span>
                          <h4 className="text-xs font-bold text-slate-900 line-clamp-1 leading-snug">{getProductName(p)}</h4>
                      ...
                      <div className="shrink-0">
                        {isOutOfStock ? (
                          <span className="text-[10px] text-rose-500 font-bold bg-rose-50 px-2 py-1 rounded-lg">Tugagan</span>
                        ) : qtyInCart === 0 ? (
                          <button
                            onClick={() => {
                              if (hasVariants || isFract) {
                                openProductModal(p);
                              } else {
                                addToCart(p);
                              }
                            }}
                            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold p-2 rounded-xl text-xs flex items-center justify-center shadow-sm"
                          >
                            <Plus className="w-4 h-4" />
                          </button>
                        ) : (
                          <div className="flex items-center bg-emerald-50 p-1 rounded-xl border border-emerald-300 gap-1.5">
                            <button
                              onClick={() => updateCartQty(p.id, isFract ? -0.1 : -1, cart.find(i=>i.product.id===p.id)?.selectedSize, cart.find(i=>i.product.id===p.id)?.selectedColor)}
                              className="w-6 h-6 bg-white text-rose-600 rounded-lg flex items-center justify-center font-bold text-xs shadow-2xs"
                            >
                              <Minus className="w-3 h-3" />
                            </button>
                            <span
                              onClick={() => isFract && openProductModal(p, qtyInCart)}
                              className={`text-[11px] font-black text-emerald-900 min-w-[28px] text-center font-mono ${isFract ? 'cursor-pointer underline' : ''}`}
                            >
                              {isFract
                                ? `${qtyInCart} ${p.unit || 'kg'}`
                                : qtyInCart}
                            ...
                            <button
                              onClick={() => addToCart(p, undefined, undefined, isFract ? 0.1 : 1)}
                              className="w-6 h-6 bg-emerald-600 text-white rounded-lg flex items-center justify-center font-bold text-xs shadow-2xs"
                            >
                              <Plus className="w-3 h-3" />
                            </button>
                          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="w-full h-32 rounded-xl overflow-hidden mb-2 bg-slate-50 relative border border-slate-200 flex items-center justify-center p-1">
                        <ProductThumbnail product={p} iconSize="w-10 h-10" imgClassName="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                      </div>
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-100">
                      ...
                      {isOutOfStock ? (
                        ...
                        >
                          ...
                          <span>{isFract ? 'Gramm / Kg Tanlash' : hasVariants ? 'Tanlash & Savat' : 'Savatga'}</span>
                        </button>
                      ) : (
                        <div className="flex items-center justify-between bg-emerald-50 p-1 rounded-xl border border-emerald-300">
                          ...
                          <span
                            onClick={() => isFract && openProductModal(p, qtyInCart)}
                            className={`text-[11px] font-black text-emerald-800 font-mono ${isFract ? 'cursor-pointer underline' : ''}`}
                          >
                            {isFract
                              ? `${qtyInCart} ${p.unit || 'kg'}`
                              : `${qtyInCart} ta`}
          ...
          {/* Load More Pagination Trigger for Smooth High Performance */}
          {!isLoadingProducts && filteredProducts.length > 0 && (
            <div className="pt-3 pb-8 flex flex-col items-center justify-center gap-1.5">
              <div className="text-[11px] text-slate-500 font-semibold">
                Ko'rsatilmoqda: <span className="font-extrabold text-emerald-800">{displayedProducts.length}</span> / <span className="font-extrabold text-slate-900">{filteredProducts.length}</span> ta tovar
              </div>
              {displayLimit < filteredProducts.length ? (
                <button
                  onClick={() => setDisplayLimit((prev) => prev + 36)}
                  className="bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white text-xs font-bold px-6 py-2.5 rounded-xl shadow-md shadow-emerald-600/20 flex items-center gap-2 transition-all cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  <span>Yana ko'proq yuklash (+36 ta)</span>
                </button>
              ) : (
                <div className="text-[10px] text-slate-400 font-bold bg-slate-100 px-3 py-1 rounded-full">
                  ✓ Barcha tovarlar ko'rsatildi ({filteredProducts.length} ta)
                </div>
              )}
            </div>
          )}
          ...
          {/* Category Directory Modal */}
            {isCategoryModalOpen && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
                <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl p-4 sm:p-6 space-y-4 max-h-[85vh] overflow-y-auto">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <Layers className="w-5 h-5 text-emerald-600" />
                      <h3 className="font-extrabold text-base text-slate-900">Barcha Toifalar</h3>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <button
                      ...
                      className={`p-3 rounded-2xl border text-left flex items-center justify-between transition-all ${
                        selectedCategory === 'all'
                          ...
                          : 'bg-slate-50 text-slate-900 border-slate-200 hover:bg-emerald-50'
                      }`}
                    >
                      <div className="font-bold text-xs">🛒 Barcha Mahsulotlar</div>
                    ...
                    {categories.map((c) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <div className="font-bold text-xs line-clamp-1">{getCategoryName(c)}</div>
                            <div className={`text-[10px] font-medium ${selectedCategory === c.id ? 'text-emerald-100' : 'text-slate-500'}`}>
                              {count} ta tovar
                            </div>
                          </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  {cart.map((item, idx) => (
                    ...
                    >
                      <div className="w-12 h-12 rounded-xl bg-slate-50 shrink-0 border border-slate-200 p-0.5 overflow-hidden flex items-center justify-center">
                        <ProductThumbnail product={item.product} iconSize="w-5 h-5" />
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-xs text-emerald-700 font-extrabold font-mono">
                          {((item.product.discountPrice || item.product.price) * item.quantity).toLocaleString()} UZS
                        </p>
                        {isFractionalProduct(item.product) && (
                          <button
                            type="button"
                            onClick={() => openProductModal(item.product, item.quantity)}
                            className="text-[10px] bg-amber-100 hover:bg-amber-200 text-amber-900 font-extrabold px-2 py-0.5 rounded-lg border border-amber-300 flex items-center gap-1 transition-all active:scale-95"
                            title="Miqdorni o'zgartirish"
                          >
                            <span>⚖️ {item.quantity} {item.product.unit || 'kg'}</span>
                            <span className="text-[9px] text-amber-700 font-semibold underline">O'zgartirish</span>
                          </button>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200">
                      <button
                        onClick={() => updateCartQty(item.product.id, isFractionalProduct(item.product) ? -0.1 : -1, item.selectedSize, item.selectedColor)}
                        className="w-7 h-7 bg-white text-slate-700 rounded-lg flex items-center justify-center font-bold text-xs shadow-xs border border-slate-200 hover:bg-slate-50 active:scale-95"
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (isFractionalProduct(item.product)) {
                            openProductModal(item.product, item.quantity);
                          }
                        }}
                        className="min-w-[48px] px-1 text-center text-xs font-black text-slate-900 bg-white border border-slate-300 rounded-md py-1 font-mono hover:border-emerald-500"
                      >
                        {isFractionalProduct(item.product)
                      ...
                      <button
                        onClick={() => addToCart(item.product, item.selectedSize, item.selectedColor, isFractionalProduct(item.product) ? 0.1 : 1)}
                        className="w-7 h-7 bg-emerald-600 text-white rounded-lg flex items-center justify-center font-bold text-xs shadow-xs hover:bg-emerald-700 active:scale-95"
                        title={isFractionalProduct(item.product) ? '+100 gr' : '+1 ta'}
                      >
                        <Plus className="w-3.5 h-3.5" />
                      </button>
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-emerald-200 rounded-3xl p-5 max-w-sm w-full space-y-4 shadow-2xl relative animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex gap-3 items-center">
              <div className="w-20 h-20 bg-slate-50 border border-slate-200 rounded-2xl p-1 shrink-0 flex items-center justify-center overflow-hidden">
                <ProductThumbnail product={selectedProductModal} iconSize="w-8 h-8" />
              </div>
              <div>
            ...
            <div className="space-y-2.5 bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <div className="flex justify-between items-center text-xs font-bold text-slate-800">
                <span>Miqdori ({selectedProductModal.unit || 'dona'}):</span>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => {
                    const minQ = isFractionalProduct(selectedProductModal) ? 0.1 : (selectedProductModal.minQuantity || 1);
                    const step = isFractionalProduct(selectedProductModal) ? 0.1 : 1;
                    setModalQty(Math.max(minQ, Math.round((modalQty - step) * 1000) / 1000));
                  }}
                  className="w-11 h-11 bg-white hover:bg-slate-100 text-slate-800 rounded-xl font-black text-lg border border-slate-300 shadow-xs flex items-center justify-center active:scale-95"
                >
                  -
                </button>
                ...
                <div className="flex-1 relative flex items-center">
                  <input
                    type="number"
                    step={isFractionalProduct(selectedProductModal) ? '0.1' : '1'}
                    min={isFractionalProduct(selectedProductModal) ? 0.1 : (selectedProductModal.minQuantity || 1)}
                    value={modalQty}
                    onChange={(e) => {
                      const minQ = isFractionalProduct(selectedProductModal) ? 0.1 : (selectedProductModal.minQuantity || 1);
                      const parsed = parseFloat(e.target.value);
                      setModalQty(isNaN(parsed) ? minQ : Math.max(minQ, Math.round(parsed * 1000) / 1000));
                    }}
                    className="w-full text-center py-2.5 pr-8 text-base font-black text-slate-900 border border-slate-300 rounded-xl bg-white focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-500 font-mono shadow-xs"
                  />
                  <span className="absolute right-3 text-xs font-bold text-slate-400 pointer-events-none">
                    {selectedProductModal.unit || 'dona'}
                  </span>
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    const step = isFractionalProduct(selectedProductModal) ? 0.1 : 1;
                    setModalQty(Math.round((modalQty + step) * 1000) / 1000);
                  }}
                  className="w-11 h-11 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black text-lg border border-emerald-600 shadow-xs flex items-center justify-center active:scale-95"
                >
                  +
                </button>
              </div>
              ...
              <div className="flex items-center justify-between bg-emerald-50 p-2.5 rounded-xl border border-emerald-200 text-xs">
                <span className="text-slate-700 font-bold">
                  {isFractionalProduct(selectedProductModal)
                    ? modalQty < 1
                      ? `⚖️ ${Math.round(modalQty * 1000)} gr (${modalQty} ${selectedProductModal.unit || 'kg'})`
                      : `⚖️ ${modalQty} ${selectedProductModal.unit || 'kg'} (${Math.round(modalQty * 1000)} gr)`
                    : `📦 ${modalQty} ${selectedProductModal.unit || 'dona'}`}
                </span>
                <span className="font-extrabold text-emerald-800 font-mono text-sm">
                  {Math.round((selectedProductModal.discountPrice || selectedProductModal.price) * modalQty).toLocaleString()} UZS
                </span>
              </div>
            </div>
