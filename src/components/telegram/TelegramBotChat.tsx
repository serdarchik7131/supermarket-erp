...
export const TelegramBotChat: React.FC<TelegramBotChatProps> = ({ onOpenMiniApp, onOpenAdmin }) => {
  ...
  const createStartButtons = (phone?: string) => {
    ...
    return [
      { text: '🛍 Xaridni boshlash', action: () => onOpenMiniApp('catalog') },
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `b_err_${Date.now()}`,
          sender: 'bot',
          text: "Kechirasiz, xabaringizni qayta ishlashda xatolik yuz berdi. Iltimos pastdagi 'Xaridni boshlash' tugmasi orqali xarid qiling.",
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          buttons: [{ text: '🛍 Xaridni boshlash', action: () => onOpenMiniApp('catalog') }],
        },
      ]);
    } finally {
      setIsLoading(false);
