        ...
        <table className="w-full text-left text-[11px] border-collapse">
          <thead className="bg-[#24275f] text-white font-extrabold border-b border-indigo-950 uppercase tracking-wider text-[10px]">
            <tr>
              ...
              <th className="p-3 border-r border-indigo-900 font-mono">
                <div className="flex items-center gap-1">
                  <LockIcon className="w-3.5 h-3.5 text-indigo-300" />
                  <span>Kirish (Login / Parol)</span>
                </div>
              </th>
          ...
          <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
            {staff.map((st) => {
              ...
              const isPasswordRevealed = !!revealedPasswordIds[st.id];
              ...
              return (
                <tr key={st.id} className="hover:bg-indigo-50/50 transition-colors">
                  ...
                  {/* Login & Password (Securely Masked) */}
                  <td className="p-3 border-r border-slate-100 font-mono">
                    <div className="flex flex-col gap-1">
                      <div className="text-indigo-950 font-black flex items-center gap-1.5 bg-indigo-50/60 px-2 py-0.5 rounded-md border border-indigo-100">
                        <KeyRound className="w-3 h-3 text-indigo-500 shrink-0" />
                        <span className="text-[10px]">Login:</span>
                        <span className="text-indigo-900 font-bold">{st.login || 'agent'}</span>
                      </div>
                      <div className="flex items-center justify-between gap-1 text-[10px] bg-slate-100/70 px-2 py-0.5 rounded-md border border-slate-200">
                        <div className="flex items-center gap-1 text-slate-600">
                          <LockIcon className="w-2.5 h-2.5 text-slate-400" />
                          <span>Parol:</span>
                          <span className="font-bold text-slate-800 tracking-wider">
                            {isPasswordRevealed ? (st.password || '••••••••') : '••••••••'}
                          </span>
                        </div>
                        <button
                          type="button"
                          onClick={() => togglePasswordReveal(st.id)}
                          className="text-slate-400 hover:text-indigo-600 p-0.5 transition-colors cursor-pointer"
                          title={isPasswordRevealed ? "Parolni yashirish" : "Parolni ko'rish"}
                        >
                          {isPasswordRevealed ? <EyeOff className="w-3 h-3 text-indigo-600" /> : <Eye className="w-3 h-3" />}
                        </button>
                      </div>
                    </div>
                  </td>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-slate-900 font-bold focus:outline-none focus:border-indigo-600"
                  >
                    <option value="content_agent">🎨 Kontent Agent (Mahsulot Nom va Rasm Studiyasi)</option>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="••••••••"
                        className="w-full bg-white border border-indigo-300 rounded-xl p-2 pr-9 text-indigo-950 font-mono font-extrabold focus:outline-none focus:border-indigo-600"
                      />
                      <button
                        type="button"
                        onClick={() => setShowModalPassword(!showModalPassword)}
                        className="absolute right-2.5 top-2.5 text-slate-400 hover:text-indigo-600 cursor-pointer"
                        title={showModalPassword ? "Parolni yashirish" : "Parolni ko'rsatish"}
                      >
