...
export const OstatkaModule: React.FC = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedBranch, selectedCategory, stockStatusFilter, pageSize]);

  const [isResetting, setIsResetting] = useState(false);

  const handleResetAllStockToZero = async () => {
    if (!window.confirm("Barcha mahsulotlar qoldig'ini 0 ga tushirishni tasdiqlaysizmi? (Bunda qoldig'i 0 bo'lgan mahsulotlar mijozlar katalogida avtomatik yashiriladi)")) {
      return;
    }
    setIsResetting(true);
    try {
      const res = await fetch('/api/products/reset-all-stock-zero', { method: 'POST' });
      if (res.ok) {
        const refreshed = await fetchProducts();
        setProducts(refreshed);
        alert("✅ Barcha mahsulotlar qoldig'i 0 qilindi! Mijozlar ko'rinishida 0-qoldiqli tovarlar yashirildi.");
      }
    } catch (e) {
  ...
  }, [products, selectedCategory, searchQuery, selectedBranch, stockStatusFilter, categories]);
  ...
  return (
    <div className="space-y-2 text-xs font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 bg-white p-2 rounded-lg border border-slate-300 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-bold px-3 py-1 rounded text-[11px] flex items-center gap-1 shadow-2xs"
            title="Barcha tovarlar qoldig'ini 0 qilish"
          >
            <AlertTriangle className="w-3.5 h-3.5" />
            <span>{isResetting ? 'Bajarilmoqda...' : "Qoldiqlarni 0 ga tushirish"}</span>
          </button>
      ...
      <div id="printable-ostatka" className="bg-white border border-slate-300 rounded-lg overflow-x-auto custom-scrollbar shadow-xs">
              ...
              return (
                <tr key={p.id} className="hover:bg-blue-50/70 transition-colors even:bg-slate-50/50">
                  <td className="p-1.5 border-r border-slate-200 flex items-center gap-2">
                    <div className="w-6 h-6 rounded bg-slate-50 border border-slate-200 shrink-0 overflow-hidden flex items-center justify-center">
                      <ProductThumbnail product={p} iconSize="w-3.5 h-3.5" />
                    </div>
                    <div>
        ...
        {/* Pagination Toolbar */}
        <div className="p-2 border-t border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 text-[11px] text-slate-600 mr-2">
              <span>Sahifada:</span>
              <select
                value={pageSize}
                onChange={(e) => setPageSize(Number(e.target.value))}
                className="bg-white border border-slate-300 rounded px-1.5 py-0.5 text-[11px] font-bold focus:outline-none"
              >
            ...
            <button
              ...
              className="px-2.5 py-0.5 bg-white border border-slate-300 rounded text-[11px] font-bold text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 cursor-pointer"
            >
              Oldingi
            </button>

            <span className="text-[11px] font-bold text-slate-800 px-1">
