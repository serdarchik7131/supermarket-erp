...
export const ProductManagement: React.FC = () => {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const totalStock: number = stockValues.reduce((acc, curr) => acc + (Number(curr) || 0), 0);
    if (activeTab === 'sellable') matchTab = totalStock > 0;
    if (activeTab === 'withdrawn') matchTab = totalStock === 0;
    ...
    // 5. Barcode Filter
    const matchBarcode = !onlyHasBarcode || (p.barcode && p.barcode !== 'None' && p.barcode.length > 5);
    ...
    return matchSearch && matchTab && matchCat && matchBrand && matchBarcode && matchPrice;
  });
                    ...
                    <td className="p-1.5 border-r border-slate-200">
                      <div className="flex items-center gap-1.5">
                        <img
                          ...
                          className="w-7 h-7 object-contain rounded shrink-0 bg-slate-50 border border-slate-200 p-0.5"
                        />
                        <div>
              ...
              >
                <div className="space-y-2">
                  <div className="h-32 bg-slate-50 rounded-xl p-2 flex items-center justify-center border border-slate-100 relative">
                    <img src={getAutoProductImage(p)} alt={p.nameUz} className="h-full object-contain" />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-3 text-xs max-h-[75vh] overflow-y-auto pr-1">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-600 block mb-1 font-semibold">Minimal buyurtma soni (dona/kg):</label>
                  <input
                    type="number"
                    min={1}
                    value={minQuantity}
                  ...
                  />
                  <span className="text-[10px] text-slate-500">Mijoz bir safarda eng kamida shuncha olishi shart</span>
                </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 bg-slate-100 p-2 rounded-lg border border-slate-200">
                    <img
                      src={image && image.trim() !== '' ? image : getAutoProductImage({ nameUz, description, categoryId })}
                      alt="Preview"
                      className="w-10 h-10 object-contain rounded bg-white border border-slate-300 p-0.5"
                    />
                    <div className="flex-1 text-[10px]">
                      {image && image.trim() !== '' ? (
                        <span className="text-emerald-700 font-bold block">✓ Qo'lda biriktirilgan rasm</span>
                      ) : (
                        <span className="text-blue-700 font-bold block">✨ Avto-Rasm: Mahsulot nomi va ta'rifiga qarab mos icon biriktirildi</span>
                      )}
                    </div>
                    {image && (
                      <button
